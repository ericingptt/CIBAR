(function(){
  ...
  class GestureController extends EventTarget{
    ...
    loop(){if(this.landmarker&&this.video&&this.video.readyState>=2){const now=performance.now();const res=this.landmarker.detectForVideo(this.video,now);const l=res.landmarks&&res.landmarks[0];if(l){const p=this.smoothPalm=this.smooth(this.smoothPalm,this.palm(l),.25);const inside=this.requireFrame?this.inFrame(p):true;this.emit('hand',{inside,palm:p});if(!inside){this.resetMotion();}else{this.lastSeen=now;if(this.mode==='scroll'||this.mode==='site')this.handleScroll(p,now);if(this.mode==='select'||this.mode==='language'||this.mode==='site')this.handleSelect(l,[...this.getTargets()],now);}}else{this.emit('hand',{inside:false});this.resetMotion();}}
